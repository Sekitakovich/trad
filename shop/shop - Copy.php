...
<?php
		$qq = array();
		$qq[] = sprintf("select tenant.name as tname,tenant.id as tid,shop.*,division.id as did,area.id as aid,currency.id as cid,currency.name as cname,staff.nickname,((cast(now() as date) between shop.ps and shop.pe) or (shop.ps=shop.pe and cast(now() as date)>=shop.ps)) as active");
