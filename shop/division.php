...
//
 ...
 //
	...
	pg_query($handle,"LOCK division IN EXCLUSIVE MODE");
...
<form action="" method="post" enctype="application/x-www-form-urlencoded" name="edit" target="_self" id="edit" onsubmit="return checkTheForm(this)">
		...
		for($a=0; $a<$qs; $a++){
			...
			$selected = sprintf("%s",$qo['id']==$parent? $__XHTMLselected:"");
...
?>
						...
						<td width="6%" class="th-edit">&nbsp;</td>
						<td width="48%" class="td-edit">&nbsp;</td>
				</tr>
				<tr>
						<td class="th-edit">名称(J)</td>
						<td class="td-edit"><input name="name" type="text" id="name" value="<?php printf("%s",$name); ?>" size="32" maxlength="64" /></td>
						<td class="th-edit">名称(E)</td>
						<td class="td-edit"><input name="rome" type="text" id="rome" value="<?php printf("%s",$rome); ?>" size="32" maxlength="64" /></td>
				</tr>
				<tr>
...
<form action="" method="post" enctype="application/x-www-form-urlencoded" name="list" target="_self" id="list">
		<table width="8%">
				<tr>
						...
						<td width="2%" class="th-edit">名称(J)</td>
						<td width="2%" class="th-edit">名称(E)</td>
						<td width="96%" class="th-edit">店舗</td>
						<td width="95%" class="th-edit">staff</td>
				</tr>
<?php
	for($a=0; $a<$qs; $a++){
		$qo = pg_fetch_array($qr,$a);
		$fullname=getDivisionName($handle,$qo['id']);
?>
				<tr id="row[<?php printf("%d",$a); ?>]" title="<?php printf("最終更新日時 %s by %s",ts2JP($qo['udate']),$qo['nickname']); ?>">
						<td class="td-edit"><label>
						...
						<td class="td-edit"><a href="division.php?mode=edit&amp;id=<?php printf("%d",$qo['id']); ?>"><img src="../images/page_edit_16x16.png" alt="修正" width="16" height="16" border="0" /></a></td>
		<td class="td-edit"><span title="<?php printf("%s",$fullname); ?>"><?php printf("%s",$qo['name']); ?></span></td>
						<td class="td-edit"><?php printf("%s",$qo['rome']); ?></td>
						<td class="td-edit"><select name="shop" id="shop" onchange="editMaster(this)">
								<?php
		$query = sprintf("select * from shop where vf=true and division='%d' order by name",$qo['id']);
...
?>
						...
						<td class="td-edit"><label>
								<select name="staff" id="staff" onchange="editMaster(this)">
<?php
		$query = sprintf("select * from staff where vf=true and '%d'=any(dset) order by nickname",$qo['id']);
		$nr = pg_query($handle,$query);
		$ns = pg_num_rows($nr);
?>
										<option value="0" selected="selected"><?php printf("%d",$ns); ?>名</option>
<?php
	for($aa=0; $aa<$ns; $aa++){
		$no = pg_fetch_array($nr,$aa);
?>
										<option value="<?php printf("%d",$no['id']); ?>"><?php printf("%s",$no['nickname']); ?></option>
<?php
	}
?>
								</select>
