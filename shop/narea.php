...
if($handle=pg_connect($pgconnect)){
	...
	{
		$query = sprintf("select count(*) from nation where vf=true and narea='%d'",$id);
...
//	Var_Dump::display($whoami);
	...
	pg_query($handle,"LOCK narea IN EXCLUSIVE MODE");
...
//--------------------------------------------------------------------
	case "wsave":
		...
		for($a=0,$b=count($idS); $b--; $a++){
			if($old[$a]!=$idS[$a]){
				$query = sprintf("update narea set weight='%d' where id='%d'",$b,$idS[$a]);
				$qr = pg_query($handle,$query);
...
//--------------------------------------------------------------------
	case "save":
		$id=$_REQUEST['id'];
		if($id==0){
			$query = sprintf("select max(id) from narea");
			$qr = pg_query($handle,$query);
?><!-- <?php printf("Query (%d) [ %s ]\n",$qr,$query); ?> --><?php
			$qo = pg_fetch_array($qr);
			$id=$qo['max']+1;
			$query = sprintf("insert into narea(id,istaff,ustaff,weight) values('%d','%d','%d','%d')",$id,$whoami['id'],$whoami['id'],$id);
			$qr = pg_query($handle,$query);
?><!-- <?php printf("Query (%d) [ %s ]\n",$qr,$query); ?> --><?php
		}
		$set = array();
		...
		if(isset($_REQUEST['delete'])){
			$query = sprintf("update narea set vf=false where id='%d'",$id);
		}
		else{
			$query = sprintf("update narea set %s where id='%d'",implode(",",$set),$id);
		}
		$qr = pg_query($handle,$query);
...
//--------------------------------------------------------------------
  ...
  $id=$_REQUEST['id'];
			...
			$query = sprintf("select narea.* from narea where narea.id='%d'",$id);
			$qr = pg_query($handle,$query);
			$qo = pg_fetch_array($qr);
			$name = $qo['name'];
			$remark = $qo['remark'];
...
//--------------------------------------------------------------------
	default:
//
		$query = sprintf("select count(*) from narea");
		$qr = pg_query($handle,$query);
		$qo = pg_fetch_array($qr);
		$wmax = $qo['count']*2;
//
		$query = sprintf("select narea.*,staff.nickname from narea,staff where narea.vf=true and narea.ustaff=staff.id order by narea.weight desc,narea.name");
		$qr = pg_query($handle,$query);
		$qs = pg_num_rows($qr);
?>
<p class="title1">地域 <a href="<?php printf("%s",$_SERVER['PHP_SELF']); ?>?mode=edit">新規登録</a></p>
<form action="" method="post" enctype="application/x-www-form-urlencoded" name="list" target="_self" id="list">
		<table width="4%">
				<tr>
						...
						<td width="2%" class="th-edit">名称</td>
						<td width="95%" class="th-edit">国</td>
...
?>
				<tr id="row[<?php printf("%d",$a); ?>]">
						...
						<td class="td-edit"><select name="nation" id="nation" onchange="editMaster(this)">
								<?php
		$query = sprintf("select * from nation where vf=true and narea='%d' order by name",$qo['id']);
		$nr = pg_query($handle,$query);
		$ns = pg_num_rows($nr);
?>
								<option value="0" selected="selected"><?php printf("%d",$ns); ?>ヶ国</option>
								<?php
	for($aa=0; $aa<$ns; $aa++){
		$no = pg_fetch_array($nr,$aa);
?>
								<option value="<?php printf("%d",$no['id']); ?>"><?php printf("%s (%s)",$no['name'],$no['kana']); ?></option>
								<?php
	}
?>
						</select></td>
