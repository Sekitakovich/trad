...
?>
		<tr registered=<?php printf("%d",$qs); ?>>
				<td class="td-edit" week="<?php printf("%d",($week+$a)%7); ?>"><?php printf("%s",dt2JP(date("Y-m-d",$us+($secDay*$a)))); ?></td>
...
//
		$__oList = array(
			...
			array('name'=>'エリア','text'=>'aw desc,aname,result desc'),
...
?>
								<option <?php printf("%s",$selected); ?> value="<?php printf("%d",$qo['id']); ?>"><?php printf("%s",$dName); ?> 以下</option>
								<?php
		}
?>
						</select></td>
